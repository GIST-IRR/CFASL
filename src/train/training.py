...
def train(train_dataset, num_epochs, model, loss_fn, args):
    ...
    if args.fp16:
        ...
        student, optimizer = amp.initialize(
    ...
    for epoch in tqdm(range(num_epochs), desc="Epoch"):
        ...
        for i, (data, class_label) in enumerate(iteration):
            ...
            if "group" in args.model_type:
                ...
                z1, z2 = (
                    outputs[1][0][: batch // 2].detach(),
                    outputs[1][0][batch // 2 :].detach(),
                )
                difference = F.mse_loss(z1, z2)
                diffsym = torch.sum(
                    torch.abs(outputs[3] - torch.eye(args.latent_dim).to(device))
                )
            ...
            if args.model_type == "controlvae":
                ...
                total_loss = (
                    reconst_err
                    + args.alpha * (orth + parl + commutative + sparse)
                    + args.beta * kld_err
                    + args.gamma * sec
                    + args.lamb * equivariant
                    + args.epsilon * equivariant_2
                )
            ...
            if (
                ...
                if "group" in args.model_type:
                    ...
                    logs["GROUP_Orthogonal"] = (
                        tr_orth - logging_orth
                    ) / args.logging_steps
                    logs["GROUP_Parallel"] = (
                        tr_parl - logging_parl
                    ) / args.logging_steps
                    logs["GROUP_equivariant_2"] = (
                        tr_equivariant_2 - logging_equivariant_2
                    ) / args.logging_steps
                    logs["GROUP_commutatve"] = (
                        tr_commutative - logging_commutative
                    ) / args.logging_steps
                    logs["GROUP_equivariant"] = (
                        tr_equivariant - logging_equivariant
                    ) / args.logging_steps
                    logs["Z_difference"] = (
                        tr_difference - logging_difference
                    ) / args.logging_steps
                    logs["Sym_difference"] = (
                        tr_diffsym - logging_diffsym
                    ) / args.logging_steps
                ...
                if (
                    ...
                    logs["TC_mutual_information"] = (
                        tr_mi - logging_mi
                    ) / args.logging_steps
                    logs["TC_total_correlation"] = (
                        tr_tc - logging_tc
                    ) / args.logging_steps
                    logging_mi = tr_mi
                    logging_tc = tr_tc
                    ...
                    logs["Commutative_group"] = (
                        tr_group - logging_group
                    ) / args.logging_steps
            ...
            if (
                ...
                or global_step == iteration_per_epoch * args.num_epoch
            ):  # save in last step
                ...
                with open(
                    os.path.join(output_dir, "attn_distribution.pickle"), "wb"
                ) as f:
