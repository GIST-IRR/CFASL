...
def qualitative(dataset, model, args):
    ...
    train_dataloader = DataLoader(
        dataset, sampler=train_sampler, batch_size=args.quali_sampling
    )
    ...
    for k, (data, class_label) in enumerate(iteration):
        with torch.no_grad():
            ...
            for i in range(data.size(0)):
                ...
                new_outputs.append(
                    reconst[i * args.interval : (i + 1) * args.interval, :, :, :]
                )
...
def changed_latent_vector_value(mean, logvar, latent_vector, interval):
    ...
    interval = interval * torch.eye(dim).to(
        device
    )  # extend to 3-D tensor # [latent_dim, latent_dim, latent_dim]
