...
def extract_group_actions(dataset, model, loss_fn, save_file, args, batch_size=64):
    ...
    group_actions, attn_dists = np.zeros(shape=(num_factor, dim, dim)), np.zeros(
        shape=(num_factor, sec * (sub_sec + 1))
    )  # np.zeros(shape=(num_factor, (dim+1)*dim))
    difference = 0.0
    ...
    for i in range(len(dataset.factor_dict)):
        ...
        transformed_z1 = torch.bmm(
            encoder_output[0][:interval_size].unsqueeze(1), group_action
        ).squeeze()
        transformed_z1 = torch.cat(
            [encoder_output[0][0].unsqueeze(0), transformed_z1], dim=0
        )
        ...
        save_image(
            torch.cat([target_img, output_1[0]]), save_dir, nrow=n_row, pad_value=1.0
        )
        save_dir = os.path.join(path, "interval_" + str(i) + ".png")
        save_image(
            torch.cat([target_img, output_2[0]]), save_dir, nrow=n_row, pad_value=1.0
        )
        ...
        transformed_z3 = torch.bmm(
            encoder_output[0][:5].unsqueeze(1), group_action
        ).squeeze()
        ...
        save_image(
            torch.cat([imgs[order_list], output_3[0][order_list]]),
            save_dir,
            nrow=args.interval,
            pad_value=1.0,
        )
        ...
        for i in range(dim):
            t_z = torch.bmm(
                transformed_z4[0],
                torch.matrix_exp(sub_symmetries[:, 0 : i + 1, :, :].sum(1)),
            )  # (5, 1, dim)
            transformed_z4.append(t_z)
        transformed_z4 = torch.cat(transformed_z4, dim=1).reshape(
            -1, dim
        )  # (5, # of sub symmetries + 1, dim) --> ( 5 * (# of sub symmetries + 1), dim
