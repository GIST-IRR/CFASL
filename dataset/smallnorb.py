...
class SmallNORBDataLoader(DisenDataLoader):
    ...
    def __init__(
        ...
        super(SmallNORBDataLoader, self).__init__(
        ...
        if self.mode == "left" or self.mode == "right":
            ...
            left, right = np.zeros((latents_classes.shape[0], 1)), np.ones(
                (latents_classes.shape[0], 1)
            )
        ...
        # load left set
            ...
            if self.mode == "stereo":
                self.data = (
                    torch.stack((left_data, right_data), dim=1).unsqueeze(1).numpy()
                )

            # load all
            else:
                self.data = (
                    torch.cat((left_data, right_data), dim=0).unsqueeze(1).numpy()
                )  # [dataset_size, 1, 96,96]
    ...
    def __getitem__(self, index):
        ...
        target = (
            self.labels[index % 24300] if self.mode == "all" else self.labels[index]
        )

        info = (
            self.latents_classes[index % 24300]
            if self.mode == "all"
            else self.latents_classes[index]
        )
    ...
    def _load(self, file_name):
        return torch.load(
            os.path.join(self.root, self.processed_folder, file_name + self.extension)
        )
    ...
    def random_sampling_for_disen_global_variance(self, batch_size, replace=False):
        ...
        indices = g.choice(
            self.data.shape[0], batch_size, replace=replace
        )  # g.choice(self.data.shape[0], batch_size, replace=replace)
    ...
    def sampling_factors_and_img(self, batch_size, num_train):
        ...
        for i in tqdm(range(num_train)):
            ...
            factors.append(
                torch.from_numpy(self.latents_classes[factor_idxs, :])
    ...
    def _check_exists(self):
        ...
        fpaths = [
            os.path.exists(
                os.path.join(self.root, self.processed_folder, f + self.extension)
            )
            for f in files
        ]
    ...
    def download(self):
        ...
        if self._check_integrity():
            ...
            for file_dict in self._flat_data_files():
                ...
                download_url(
                    url, os.path.join(self.root, self.raw_folder), gz_filename, md5
                )

                print("# Extracting data {}\n".format(filename))

                with open(fpath, "wb") as out_f, gzip.GzipFile(gz_fpath) as zip_f:
    ...
    def _read_image(file_pointer, height, width):
        ...
        image = struct.unpack(
            "<" + height * width * "B", file_pointer.read(height * width)
        )
